...
import { useStoreAllPopular } from '/src/store/allPopular';

describe('useStoreAllPopular', () => {
  let pinia;
  ...
  beforeAll(() => {
    ...
    pinia = createPinia();
    app.use(pinia);
    pinia.useStore = useStoreAllPopular;
  });

  it('should fetch popular movies and update state', async () => {
  ...
  it('should handle errors during fetchPopularMovies', async () => {
    ...
    // Dispara a ação fetchPopularMovies
    await store.fetchPopularMovies();
