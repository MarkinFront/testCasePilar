...
const profilePath = computed(() => "https://image.tmdb.org/t/p/w400/" + person.value.profile_path)
const personAge = computed(() => Math.floor((new Date() - new Date(person.value.birthday).getTime()) / 3.15576e+10))

const formatDate = computed(() => date => new Date(date).toLocaleDateString('el-GR'))

const knownForMovies = computed(() => {
  if (personCredits.value.cast)
    return personCredits.value.cast.sort((a, b) => b.popularity - a.popularity).slice(0, 5)
      .map(personCredit => {
  ...
  <div v-if="renderTemplate">
      ...
      <div class="container mx-auto px-4 py-16 flex flex-col lg:flex-row">
        <div class="flex-none mx-auto">
          ...
          <ul class="flex items-center mt-4">
            <li v-if="personSocial.facebook_id">
              <a :href="`https://facebook.com/${personSocial.facebook_id}`" title="Facebook">
                <svg class="fill-current dark:text-gray-400 dark:hover:text-white text-gray-500 hover:text-white w-6"
                  viewBox="0 0 448 512">
                  <path
            ...
            <li v-if="personSocial.instagram_id" class="ml-6">
              <a :href="`https://instagram.com/${personSocial.instagram_id}`" title="Instagram">
                <svg class="fill-current dark:text-gray-400 dark:hover:text-white text-gray-500 hover:text-white w-6"
                  viewBox="0 0 448 512">
                  <path
            ...
            <li v-if="personSocial.twitter_id" class="ml-6">
              <a :href="`https://twitter.com/${personSocial.twitter_id}`" title="Twitter">
                <svg class="fill-current dark:text-gray-400 dark:hover:text-white text-gray-500  hover:text-white w-6"
                  viewBox="0 0 512 512">
                  <path
            ...
            <li v-if="person.homepage" class="ml-6">
              <a :href="person.homepage" title="Website">
                <svg class="fill-current dark:text-gray-400 dark:hover:text-white text-gray-500 hover:text-white w-6"
                  viewBox="0 0 496 512">
                  <path
        ...
        <div class="lg:ml-24">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-8">
            <div v-for="knownForMovie in knownForMovies" :key="knownForMovie.id" class="mt-4">
              <router-link :to="knownForMovie.linkToPage">
                <img :src="knownForMovie.posterPath" :alt="`Poster of movie ${knownForMovie.title}`"
                  class="hover:opacity-75 transition ease-in-out duration-150">
                <span class="text-sm leading-normal block text-gray-400 hover:text-white mt-1">{{ knownForMovie.title
                }}</span>
              </router-link>
            </div>
          </div>
    ...
    <div class="credits">
      <div class="container mx-auto px-4 py-16">
        <h2 class="dark:text-amber-300 text-red-500 text-4xl font-semibold">Filmography</h2>
        <ul class="list-disc leading-loose pl-5 mt-8">
          <li v-for="credit in credits" :key="credit.id">
            {{ credit.releaseYear }} &middot;
            <strong>
              <router-link :to="credit.linkToPage" class="hover:underline">{{ credit.title }}</router-link>
            </strong>
            as {{ credit.character }}
          </li>
        </ul>
